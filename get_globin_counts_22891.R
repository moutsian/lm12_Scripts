...
counts=read.table("/lustre/scratch115/projects/paxgene/SalmonOutput_newruns/IU_22891_sorted.transcript_to_gene.counts",head=T) # this is used for the counts, so library name matters.
colnames(counts)=gsub('X2', '2', colnames(counts))
colnames(counts)=gsub('_IU.cdna_idx.sorted.raw', '', colnames(counts))
