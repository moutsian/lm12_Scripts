...
if (!file.exists(ens84_txdb_pth)) {
    ens84_txdb_pth <- ensDbFromGtf(gtf="/lustre/scratch115/projects/pneumo-inf/QCtesting/Homo_sapiens.GRCh38.84.gtf.gz")
}
...
which <- GRanges(paste("chr",CHR,":1-300000000",sep=""))
...
write.table(output_file,paste(files[i],".gene_quant.chr",CHR,".txt",sep=""),quote=F,row.names=F,col.names=T,sep="\t")
}
