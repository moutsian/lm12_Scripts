...
bsub -q normal -J $loci.$utrait.$study -R"select[mem>3000] rusage[mem=3000]" -M3000 -o $results_dir/$study.$utrait.$loci.$chr.$startLoci.$endLoci.out -e $log_dir/$study.$utrait.$loci.$chr.$startLoci.$endLoci.err 	/nfs/team152/javi/ToolBox/snptest_v2.5_linux_x86_64_static/snptest_v2.5  -data $Data_dir/$chr.gen.gz  $Sample_dir/${study}_with_pcs.sample -o $results_dir/$study.$utrait.$loci.$chr.$startLoci.$endLoci.$round.assoc -frequentist 1 -method score -pheno bin1 -cov_all_continuous -condition_on $topSNP -range $startLoci-$endLoci -exclude_samples /lustre/scratch113/projects/crohns/2015jan20/GWAS_imputation/assoc/gwas3-$ltrait-assoc-sample-exclusion.txt -exclude_snps $Javi_dir/SNPsPassFilters/$chr.$ltrait.snps.NoPassFilters.OnlyIDs.txt 
