...
shellcommand=paste(" cat ~lm12/IBD_conditional/step2.*dataset_B.NOVEL.round6.with_info_on_previous_hits_from_all_PAPERS.txt >  ~lm12/IBD_conditional/step2.ALL.dataset_B.NOVEL.round6.with_info_on_previous_hits_from_all_PAPERS.txt",sep="")

system(shellcommand)
results=as.matrix(read.table("~lm12/IBD_conditional/step2.ALL.dataset_B.NOVEL.round6.with_info_on_previous_hits_from_all_PAPERS.txt",sep="\t",head=T))
...
write.table(resultso_plus,"~lm12/IBD_conditional/step2.ALL.dataset_B.NOVEL.round6.with_info_on_previous_hits_from_all_PAPERS.auto.txt",quote=F,col.names=T,row.names=F,sep="\t")
