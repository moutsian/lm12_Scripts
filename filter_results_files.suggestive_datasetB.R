...
if(TRAIT=="IBD"){
	...
	print(paste("i: ",i,sep=""))
	shellcmd=paste("zgrep ",as.character(results[i,3])," /lustre/scratch113/projects/crohns/2015jan20/GWAS_imputation/assoc/ALL/QC/",as.character(results[i,1]),".txt.gz|grep ",as.character(results[i,4])," | grep ",as.character(results[i,5]),sep="")
	...
	write.table(results[to_include==1,],paste("/lustre/scratch115/teams/anderson/ibd_conditional/UK_only_analysis_B_results/UK_only_analysis_B.",TRAIT,".1e_05.filtered.txt",sep=""),quote=F,row.names=F,col.names=T)
