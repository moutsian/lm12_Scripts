...
/nfs/team152/javi/ToolBox/snptest_v2.5_linux_x86_64_static/snptest_v2.5  -data $Data_dir/$chr.gen.gz  $Sample_dir/${study}_with_pcs.sample -o $results_dir/$study.$utrait.$chr.$startLoci.$endLoci.assoc -frequentist 1 -method score -pheno bin1 -cov_all_continuous -condition_on "$conditional_snp1" "$conditional_snp2" -range $startLoci-$endLoci -exclude_snps $Javi_dir/SNPsPassFilters/$chr.$utrait.snps.NoPassFilters.OnlyIDs.txt
