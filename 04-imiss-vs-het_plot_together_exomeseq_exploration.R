inputpathname = "tili.i3.site_QC"

cases=read.table("list_of_samples_to_keep.cases.txt")
ctrls=read.table("list_of_samples_to_keep.ctrls.clean.txt")
...
idx_cases=which(imiss[,1]%in%cases[,1])
idx_clean_ctrls=which(imiss[,1]%in%ctrls[,1])
tmp=rbind(cases,ctrls)
idx_added_ctrls=which(!imiss[,1]%in%tmp[,1])
...
points(imiss$logF_MISS[idx_clean_ctrls], het$meanHet[idx_clean_ctrls],col="gray",pch=20)
points(imiss$logF_MISS[idx_cases], het$meanHet[idx_cases],col="darkred",pch=20)
...
legend(x="topright",c("clean ctrls","cases","other ctrls"),pch=20,col=c("gray","darkred","darkblue"))
