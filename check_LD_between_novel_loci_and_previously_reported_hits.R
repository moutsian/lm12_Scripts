...
oldidx=which(as.numeric(previousgw[,3])==chr & abs(as.numeric(as.character(previousgw[,4]))-as.numeric(positions[i]))<5000000)
	if(length(oldidx)>0){
	unique_old=unique(as.character(previousgw[oldidx,4]))
...
print(paste("no putatively novel hits in chromsome ",chr,sep=""))
}
}
#END