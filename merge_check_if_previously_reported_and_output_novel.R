...
shellcommand=paste(" cat /lustre/scratch115/teams/anderson/ibd_conditional/final_meta_analysis_C_results/",toupper(trait),"/step2.",toupper(trait),"*.txt >  ~lm12/IBD_conditional/step2.",toupper(trait),".all",sep="")
system(shellcommand)
hitsC=as.matrix(read.table(paste("~lm12/IBD_conditional/step2.",toupper(trait),".all",sep=""),sep="\t",head=T))
...
write.table(hitsCplus_all,paste("~lm12/IBD_conditional/step2.",trait,".all.round6.with_info_on_previous_hits_from_all_PAPERS.txt",sep=""),sep="\t",col.names=T,quote=F,row.names=F)
...
shellfreqin=paste(" sh /lustre/scratch115/teams/anderson/ibd_conditional/get_our_freq.sh ",toupper(trait2),as.character(novel[i,1]),as.character(novel[i,3]),sep=" ")
...
write.table(novel_plus,paste("~lm12/IBD_conditional/step2.",trait,".NOVEL.round7.withEllinghaus.with_info_on_previous_hits_from_all_PAPERS.txt",sep=""),sep="\t",col.names=T,quote=F,row.names=F)
