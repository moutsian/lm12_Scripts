...
trait=tolower("uc")
TRAIT=toupper(trait)
new=read.table(paste("/lustre/scratch115/teams/anderson/ibd_conditional/UK_only_analysis_B_results/UK_only_analysis_B.",TRAIT,".5e_08.filtered.txt",sep=""),head=T)
...
old_B_res=read.table(paste("/lustre/scratch115/teams/anderson/ibd_conditional/UK_only_analysis_B_results_beforePCcorrection/",TRAIT,"/results.",TRAIT,".",chr,".txt",sep=""),head=T)
...
write.table(LD,paste("/lustre/scratch115/teams/anderson/ibd_conditional/UK_only_analysis_B_results/",TRAIT,"/results.",TRAIT,".",chr,".txt",sep=""),col.names=T,row.names=F,quote=F)
}
}
#END