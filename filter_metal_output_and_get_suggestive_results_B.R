...
idxGWAS1=which(as.character(resGWAS1[,2])%in%as.character(metares_f2[,1]))
GWAS1filt=resGWAS1[idxGWAS1,c(2,4,42)]
GWAS1filt=GWAS1filt[!duplicated(GWAS1filt[,1]),]
GWAS1filto=GWAS1filt[order(GWAS1filt[,1]),]

idxGWAS2=which(as.character(resGWAS2[,2])%in%as.character(metares_f2[,1]))
GWAS2filt=resGWAS2[idxGWAS2,c(2,4,42)]
GWAS2filt=GWAS2filt[!duplicated(GWAS2filt[,1]),]
GWAS2filto=GWAS2filt[order(GWAS2filt[,1]),]
...
write.table(metares_fin,paste("/lustre/scratch115/teams/anderson/ibd_conditional/UK_only_analysis_B_results/",toupper(trait),"/",chr,"-meta.filtered.LM.txt",sep=""),quote=F,row.names=F,col.names=T,sep="\t")
