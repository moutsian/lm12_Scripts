setwd("C:/Academic/SANGER/LAIV/PC_plots_from_bam_alignments/")
#pc_unordered=read.table("MDS.1-22.txt",head=T)
pc_unordered=read.table("MDS.1-22.logFC.coding_genes_only.txt",head=T)
#pc_unordered=read.table("MDS.Javi.clean.logFC.coding_genes_only.txt",head=T) #note that these are from clean kallisto quantifications by Javi.
info_cond=read.table("C:/Academic/SANGER/LAIV/PC plots/listSeqFiles.Condition",head=T)
info_tissue=read.table("C:/Academic/SANGER/LAIV/PC plots/listSeqFiles.Tissue",head=T)
...
idx=matrix(ncol=dim(pc_unordered)[1],0)
for(i in 1:length(idx)){
myidx=which(pc_unordered[,1]==info_tissue[i,1])
if(length(myidx)==0){
print(paste("error- sample ",info_tissue[i,1]," not found.",sep=""))
}else{
idx[i]=myidx
}
}
pc=pc_unordered[idx,c(2:11)]
par(mfrow=c(2,4))
#1
...
points(pc[blood_minus5,PCA],pc[blood_minus5,PCB],col="darkred",pch=19)
points(pc[blood_plus2,PCA],pc[blood_plus2,PCB],col="red",pch=19)
#title("PCs from aligned BAMs, all chromosomes, samples coloured by tissue type (red vs blue) and condition (light vs dark)",outer=T)
title("MDS, QC+ files, samples coloured by by tissue type (red vs blue) and condition (light vs dark)\nquantification from R using aligned BAMs, coding genes & processed transcripts only, metric=logFC",outer=T)
...
points(pc[well8,PCA],pc[well8,PCB],col="chocolate",pch=19)
points(pc[well9,PCA],pc[well9,PCB],col="pink",pch=19)
title("PCs, aligned bams, chr 14-20, samples coloured by well",outer=T)
...
allpchs=sample(c(15:19,22:25),dim(tmp),replace=TRUE)
BG=rep(c("pink","yellow","green","purple","darkblue","chocolate","black","gray","cyan","darkred"),2)
par(mfrow=c(2,2),oma=c(0,0,2,0))
#1
PCA=1
PCB=2
...
print(idx)
points(pc[idx,PCA],pc[idx,PCB],col=allcols[i],pch=allpchs[i],cex=1.2,bg=BG,lwd=2)
}
...
#title("PCs, aligned BAMs, chr 14-20, coloured by individual ID")
...
#title("PCs, all chroms, quantification from aligned BAMs,samples coloured by individual ID",outer=T)
title("MDS, QC+ files, samples coloured by individual ID\nquantification from R using aligned BAMs, coding genes & processed transcripts only, metric=logFC",outer=T)


par(mfrow=c(1,3))
PCA=2
PCB=4
plot(pc[,PCA],pc[,PCB],pch=19,col="white",xlab=list("PC2",cex=1.2), ylab=list("PC4",cex=1.2))
for(i in 1:dim(tmp)[1]){
idx=which(moreinfo[,7]==names(tmp)[i])
print(idx)
points(pc[idx,PCA],pc[idx,PCB],col=allcols[i],pch=allpchs[i])
}
title("PCs, samples coloured by individual ID",outer=F)
#2
PCA=2
PCB=4
plot(pc[,PCA],pc[,PCB],pch=19,col="gray",xlab=list("PC2",cex=1.2), ylab=list("PC4",cex=1.2))
...
points(pc[exp_carriers,PCA],pc[exp_carriers,PCB],col=COL2,pch=19)
points(pc[exp_carriers_before,PCA],pc[exp_carriers_before,PCB],col=COL3,pch=19)
title("MDS, QC+ files, samples coloured by carrier status\nquantification from R using aligned BAMs, coding genes & processed transcripts only, metric=logFC",outer=T)
legend(-0.2,-0.4,col=c(COL1,COL2,COL3),c("natural carriers","exp.carriers on +2","exp.carriers on -5"),pch=19,cex=0.75,pt.cex=1,ncol=1,title.adj=0.5,y.intersp=0.65,inset=c(0,-0.2))
