...
suggestive=read.table(paste("/lustre/scratch115/teams/anderson/ibd_conditional/final_meta_analysis_C_results/final_meta_analysis_C.",trait,".1e_05.filtered.txt",sep=""),head=T) #change this to contain the filtered version once you have it!
for(chr in 1:22){
...
pvals=matrix(ncol=3,nrow=dim(dataout)[1],NA);
for(i in 1:dim(dataout)[1]){
idx=which(as.numeric(as.character(sugg[,2]))==as.numeric(as.character(dataout[i,3])))
if(length(idx)==1){
pvals[i,]=as.matrix(sugg[idx,14:16])
}else{
print("lathos!")
}
}
colnames(pvals)=colnames(sugg)[14:16]
...
idx=which(as.numeric(as.character(sugg[,2]))==as.numeric(as.character(dataout[3])))
pvals=as.matrix(sugg[idx,14:16])
names(pvals)=colnames(sugg)[14:16]
