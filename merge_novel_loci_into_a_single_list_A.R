...
shellcommand=paste(" cat ~lm12/IBD_conditional/step2.*dataset_A.NOVEL.round2.with_info_on_previous_hits_from_all_PAPERS.txt >  ~lm12/IBD_conditional/step2.ALL.dataset_A.NOVEL.round2.with_info_on_previous_hits_from_all_PAPERS.txt",sep="")

system(shellcommand)
results=as.matrix(read.table("~lm12/IBD_conditional/step2.ALL.dataset_A.NOVEL.round2.with_info_on_previous_hits_from_all_PAPERS.txt",sep="\t",head=T))
...
END=dim(resultso)[1]
old_dim=0
while(old_dim!=dim(resultso)[1]){
old_dim=dim(resultso)[1]
for(i in 2:END){
if (i> END){
break}
...
write.table(resultso_plus,"~lm12/IBD_conditional/step2.ALL.dataset_A.NOVEL.round2.with_info_on_previous_hits_from_all_PAPERS.auto.txt",quote=F,col.names=T,row.names=F,sep="\t")
