...
#alelle="0301"
logreg_12p_nounc_nogender_oct14_SNP_INT = function(case_file,control_file,
...
source(paste(SCRIPTDIR,"GWAS_12p_nounc_nogender_april15.R",sep=""))
...
log_reg_results = matrix(nrow=to-from+1,ncol=45)
for(i in from:to){
cat(i," ");
	...
	DATA[GENO==2,3]=1
	DATA[GENO==0,1]=1
	log_reg_results[i-from+1,]=as.matrix(gwas12_nounc_noG_april15_INT(interaction_term,status,COND_DATA,COND_DATA2,COND_DATA3,COND_DATA4,COND_DATA5,COND_DATA6,COND_DATA7,COND_DATA8,COND_DATA9,CSNPDATA,CSNPDATA2,DATA,PCMATRIX))
	} 
}
cat("\n");

rownames(log_reg_results)= SNP_IDs[from:to]
colnames(log_reg_results)=c("mu","gamma1","gamma1HOM","gamma2","gamma2HOM","gamma3","gamma4REC","gamma5","gamma6","gamma7","gamma8","gamma9","gamma10","gamma11","gamma12",
"se1","se1HOM","se2","se2HOM","se3","se4REC","se5","se6","se7","se8","se9","se10","se11","se12",
"pval1","pval1HOM","pval2","pval2HOM","pval3","pval4REC","pvalue5","pvalue6","pvalue7","pvalue8","pvalue9","pvalue10","pvalue11","pvalue12",
"dev_NULL","dev_M")
