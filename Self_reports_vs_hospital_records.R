...
Disease="malabsorption/coeliac disease" #examples: "crohns disease" "irritable bowel syndrome" "sclerosing cholangitis" "malabsorption/coeliac disease"
...
# In the meantime, I am just checking what are the diseases that these 741 individuals are self-reporting (note that 741 is the number for IBS only, it changes for other diseases).
...
outfile_contrast=paste("/lustre/scratch115/teams/anderson/ukbiobank/plots/","Self_reported_traits_for_people_with_medical_record_of_",tmp,".",MIN_COUNTS,".pdf",sep="")
...
legend("topleft",pch=19,col=c("darkblue","red"),c("Prevalence in the discordant individuals","Prevalence in UKB"),cex=0.8)
mtext(paste("(Min of ",MIN_COUNTS," in the discordant individuals.)",sep=""),3,cex=0.7)
if(printtopdf){dev.off()}

outfile_contrast=paste("/lustre/scratch115/teams/anderson/ukbiobank/plots/","Self_reported_traits_for_people_with_medical_record_of_",tmp,".",MIN_COUNTS,".less_prev.pdf",sep="")
if(printtopdf){pdf(outfile_contrast)}
par(mar=c(15,4,4,2))
